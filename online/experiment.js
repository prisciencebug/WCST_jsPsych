...
var subjectId = {
  ...
  questions: [
    {
      prompt: "Bitte geben Sie unten Ihre Versuchspersonennummer ein: ",
      required: true,
    },
  ],
...
jsPsych.init({
  ...
  on_close: function () {
    ...
    jsPsych.data
      .get()
      .localSave("csv", `WCST_subject_${subjID}_quitted_output.csv`);
  },
  on_data_update: function () {
    if (jsPsych.data.get().last(1).values()[0].is_trial === true) {
      ...
      if (
        nMinus1Trial.trial_number > 1 &&
        nMinus1Trial.correct === false &&
        nMinus2Trial.correct_in_row > 4 &&
        nMinus2Trial.correct_in_row !== 10
      ) {
      ...
      if (nMinus3Trial) {
        if (
          nMinus1Trial.correct &&
          nMinus2Trial.correct &&
          nMinus3Trial.correct
        ) {
